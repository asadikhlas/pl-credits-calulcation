 ...
 */
var getExtGsmCharactersCount = function getExtGsmCharactersCount(text) {
  ...
  return matches ? _toConsumableArray(matches).length : 0;
};
 ...
 */
var getCreditsCostAndStats = function getCreditsCostAndStats(text, unicodeEncoding, senderNumber) {
  var subscribersCount = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 1;
  var imagesAndVideos = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : [];
  var withSegmentLogic = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;
  // Assign full text to a variable
  var fullText = text;

  // Calculate the count of extended GSM characters based on encoding
  var extGsmCharactersCount = unicodeEncoding ? 0 : getExtGsmCharactersCount(fullText);

  // Calculate total characters including extended GSM characters
  var characters = (fullText === null || fullText === void 0 ? void 0 : fullText.length) + extGsmCharactersCount;

  // Set initial max characters per segment based on encoding
  var maxPerSegment = unicodeEncoding ? 70 : 160;
  ...
  var smsCount = Math.ceil(characters / maxPerSegment);

  // Calculate the number of MMS (1 if there are images/videos, else 0)
  var mmsCount = (imagesAndVideos === null || imagesAndVideos === void 0 ? void 0 : imagesAndVideos.length) > 0 ? 1 : 0;

  // Calculate the total cost of sending the message
  var calculatedCredits = senderNumber && subscribersCount ? subscribersCount * ((mmsCount ? 0 : smsCount) * (senderNumber === null || senderNumber === void 0 ? void 0 : senderNumber.sms_cost) + mmsCount * (senderNumber === null || senderNumber === void 0 ? void 0 : senderNumber.mms_cost)) : 0;

  // Generate a warning for messages exceeding the maximum character limit
  var tooLongWarning = characters > 1600 ? "Max characters count 1600 exceed." : null;
  ...
  return {
    ...
    smsCount: smsCount,
    mmsCount: mmsCount,
    tooLongWarning: tooLongWarning
  };
};
module.exports = {
  getCreditsCostAndStats: getCreditsCostAndStats,
  getExtGsmCharactersCount: getExtGsmCharactersCount
};